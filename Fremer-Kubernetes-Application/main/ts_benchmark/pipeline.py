    ...
    for model_factory, result_itr, model_save_name in zip(
        ...
        for i, result_df in enumerate(result_itr.collect()):
            ...
            settings = "{}_{}_sl{}_pl{}".format(
                model_config['models'][i]['model_name'], 
                data_config['data_name_list'][i][:-4],
                model_config['models'][i]['model_hyper_params']['seq_len'],
                model_config['models'][i]['model_hyper_params']['horizon'],
            )
            if formal_exp:
                result_file = "./TotalResults.txt"
            else:
                result_file = "./TotalResults-Test.txt"
            with open(result_file, 'a') as file:
                file.write(settings + '\n')
                file.write("mse_norm:{:.6f} mae_norm:{:.6f} smape:{:.6f}".format(
                    result_df['mse_norm'].values[i], result_df['mae_norm'].values[i], result_df['smape'].values[i])
                )
                file.write('\n\n')
