...
class Fremer(ModelBase):
    ...
    def forecast_fit(
        ...
        # -------------------------------------------------------------
        # ✅ Evaluate model performance on validation set (if available)
        # -------------------------------------------------------------
        if train_ratio_in_tv != 1:
            ...
            try:
                val_pred = self.forecast(horizon=horizon, train=valid_data)
            except Exception as e:
                print("⚠️ Forecast failed on validation data:", e)
                val_pred = None

            if val_pred is not None:
                # Get actual (true) values for comparison
                val_true = valid_data.values[-horizon:]
                ...
                # Optional: print sample actual vs predicted values
                comparison = pd.DataFrame({
                    "Actual": val_true.flatten(),
                ...
                try:
                    ...
                    plt.plot(val_true.flatten(), label="Actual")
        ...
        # -------------------------------------------------------------
        save_dir = os.path.join("./checkpoints", settings)
