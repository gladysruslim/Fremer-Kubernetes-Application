...
class CompEncoderBlock(nn.Module):
    ...
    def __init__(self, configs, extended=True):
        ...
        self.index = get_frequency_modes(self.ori_len, modes=64)
        self.d_ff = 256
        self.d_model = 128
        self.emb = ComplexLinear(configs.enc_in, self.d_model)
        self.dropout = nn.Dropout(configs.dropout)
        self.projection = ComplexLinear(self.d_model, configs.enc_in)
    ...
    def forward(self, x):  # x.shape: [B, L:seq_len/2, n_vars] dtype: torch.cfloat
        ...
        x = self.encoder(x) # x.shape: [B, L, n_vars]
        x = self.projection(x)
